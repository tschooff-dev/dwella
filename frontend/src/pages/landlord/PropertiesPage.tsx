  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const propRes = await apiFetch('/api/properties', {
        method: 'POST',
        body: JSON.stringify({ name, address, city, state, zip, description: description || undefined, landlordId }),
      })
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={addUnit} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">
                + Add unit
              </button>
            </div>
            {units.map((unit, i) => (
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={e => updateUnit(i, 'rentAmount', e.target.value)}
                        placeholder="1500"
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => updateUnit(i, 'squareFeet', e.target.value)}
                      placeholder="750"
                      className="input"
                    />
                  </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={e => updateUnit(i, 'depositAmount', e.target.value)}
                      placeholder="1500"
                      className="input pl-6"
                    />
                  </div>
...
export default function PropertiesPage() {
  ...
  function handleUnitAdded(unit: Unit) {
    setProperties(prev =>
      prev?.map(p =>
        p.id === addUnitFor?.id ? { ...p, units: [...p.units, unit] } : p
      ) ?? null
    )
    setAddUnitFor(null)
  }
              ...
              <div key={property.id} className="card overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4 text-right">
                    ...
                    <button className="btn-secondary text-xs" onClick={() => setAddUnitFor(property)}>+ Add Unit</button>
                  </div>
                </div>
                {property.units.length === 0 ? (
                  <div className="px-5 py-4 text-xs text-gray-400">
                    No units yet.{' '}
                    <button className="text-indigo-600 hover:underline" onClick={() => setAddUnitFor(property)}>Add a unit</button>
                  </div>
                ) : (
